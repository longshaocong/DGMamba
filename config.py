...
# could be overwritten by command line argument
_C.MODEL.PRETRAINED = './utils/vssmtiny_dp02_ckpt_epoch_258.pth'
...
def update_config(config, args):
    ...
    if _check_args('alpha'):
        if args.algorithm != 'DGMamba':
            config.alpha = -1
        else:
            config.alpha = args.alpha

    # output folder
    config.OUTPUT = os.path.join(config.OUTPUT, config.domains[config.test_envs[0]])
